...
export async function getContentBasedScores(
  ...
  if (profile.totalWatched === 0) return new Map();
  ...
  for (const film of candidateFilms) {
    ...
    if (film.genres) {
      if (Array.isArray(film.genres)) filmGenres = film.genres;
      else if (typeof film.genres === 'string') {
        try { filmGenres = JSON.parse(film.genres); } catch { filmGenres = []; }
      }
    }
    ...
    if (filmGenres.length > 0) totalScore = totalScore / filmGenres.length;
    else totalScore = 0;
